...
var _preserve = { "username": _NEOLOGS._username, "password": _NEOLOGS._password, "id_application": _NEOLOGS._id_application };
...
$("body").off("click", ".btn-api_search").on("click", ".btn-api_search", function () {
	...
	if (_FUNCTIONS.onEvalSearchAPI(_params)) {
		setActive();
		_NEOLOGS.onMonitoring(_params).then(function (data) {
...
function InitStateAPI() {
	setActive();
	_NEOLOGS.onState().then(function (data) {
...
function setActive() {
	_NEOLOGS._username = _active.username;
	_NEOLOGS._password = _active.password;
	_NEOLOGS._id_application = _active.id_application;
}

function getPreserve() {
	_NEOLOGS._username = _preserve.username;
	_NEOLOGS._password = _preserve.password;
	_NEOLOGS._id_application = _preserve.id_application;
}
