...
var _preserve = { "username": _NEORPA._username, "password": _NEORPA._password, "id_application": _NEORPA._id_application };
...
$("body").off("click", ".btn-api_search").on("click", ".btn-api_search", function () {
	...
	if (_FUNCTIONS.onEvalSearchAPI(_params)) {
		setActive();
		_NEORPA.onMonitoring(_params).then(function (data) {
...
function InitStateAPI() {
	setActive();
	_NEORPA.onState().then(function (data) {
...
function setActive() {
	_NEORPA._username = _active.username;
	_NEORPA._password = _active.password;
	_NEORPA._id_application = _active.id_application;
}

function getPreserve() {
	_NEORPA._username = _preserve.username;
	_NEORPA._password = _preserve.password;
	_NEORPA._id_application = _preserve.id_application;
}
